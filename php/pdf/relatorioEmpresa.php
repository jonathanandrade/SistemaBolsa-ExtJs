...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$this->SetFont('', 'B');
		// Header
		$w = array(19, 40, 108, 100); // Tamanho de cada coluna
		...
		foreach($data as $row) {
			...
			$this->Cell($w[3], 6, $row['atividade'], 'LR', 0, 'C', $fill);
			//$this->Cell($w[4], 6, $row['login'], 'LR', 0, 'C', $fill);
	...
	//load data
	$queryString = "SELECT idempresa, sigla, descricao, atividade, login from empresa where login = '$login'" or die(mysql_error());		
	//consulta total de linhas na tabela
	$queryTotal = mysql_query("SELECT count(*) as num FROM empresa where login = '$login'") or die(mysql_error());
...
$pdf->SetHeaderData(PDF_HEADER_LOGO, 80, 'RELATÓRIO DE EMPRESAS', 'Relatório Gerado: ' . $date . ' | Usuário: ' . $_SESSION['login'], array(0,64,255), array(0,64,128));
...
$pdf->AddPage();
//Column titles
$header = array('ID', 'Sigla', 'Descrição', 'Atividade');
// print colored table
