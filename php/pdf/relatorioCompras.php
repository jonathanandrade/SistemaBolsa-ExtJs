...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$this->SetFont('', 'B');
		// Header
		$w = array(15, 20, 113, 30, 30, 30, 30); // Tamanho de cada coluna
		...
		foreach($data as $row) {
			...
			$this->Cell($w[3], 6, $row['dataCompra'], 'LR', 0, 'C', $fill);
			$this->Cell($w[4], 6, $row['quantidade'], 'LR', 0, 'C', $fill);
			$this->Cell($w[5], 6, $row['valorUnitario'], 'LR', 0, 'C', $fill);
			$this->Cell($w[6], 6, $row['total'], 'LR', 0, 'C', $fill);
			//$this->Cell($w[7], 6, $row[''], 'LR', 0, 'C', $fill);
...
$pdf->SetHeaderData(PDF_HEADER_LOGO, 80, 'RELATÓRIO DE COMPRAS', 'Relatório Gerado: ' . $date . ' | Usuário: ' . $_SESSION['login'], array(0,64,255), array(0,64,128));
...
$pdf->AddPage();
//Column titles
$header = array('ID', 'Ativo', 'Descrição', 'Data Compra', 'Qtd', 'Vlr. Unit', 'Total');
// print colored table
